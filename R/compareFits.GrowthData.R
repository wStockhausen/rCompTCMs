...
compareFits.GrowthData<-function(objs,
    ...
    for (d in datasets){
        ...
        p <- ggplot(mdfrp,aes_string(x='z',y='val',colour='case',shape='y'));
        ...
        p <- ggplot(mdfrpn,aes_string(x='z',y='val',colour='case',shape='y'));
        ...
        p <- ggplot(mdfrpz,aes_string(x='z',y='val',colour='case',shape='y'));
        p <- p + geom_point(position=pd);
        p <- p + geom_abline(slope=0,linetype=2);
