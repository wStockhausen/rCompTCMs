#'
#' @title Render a document of comparison plots for model fits to aggregated fishery abundance/biomass data
#'
#' @description Function to render a document of comparison plots for model fits to
#' aggregated fishery abundance/biomass data.
#'
...
#' @param rmd - Rmd file to process (defalut="rmd/modelComparisons.ModelFits.ACD.Fisheries.Rmd")
...
#'
#' @details Resulting document title will be of the form "ModelComparisons.ModelFits.ACD.Fisheries.mmm.ext",
#' where "ext" is the appropriate file extension and "mmm" is a dash-separated string of model names.
#'
#' @export
#'
modelComparisons.ModelFits.ACD.Fisheries<-function(
        models,
        plot1stObs=TRUE,
        output_format=c("word_document","pdf_document"),
        output_dir=getwd(),
        rmd=system.file("rmd/modelComparisons.ModelFits.ACD.Fisheries.Rmd",package="rCompTCMs"),
  ...
  output_file<-paste0("ModelComparisons.ModelFits.ACD.Fisheries.",mmm,".",ext);
  title<-paste0("Model Model Comparisons: Fits to Aggregated Fishery Catch Data\n",mmv);
