#'
#' @title Render a document of comparison plots for model fits to aggregated abundance/biomass data
#'
#' @description Function to render a document of comparison plots for model fits to
#' aggregated abundance/biomass data.
#'
...
#'
#' @details Resulting document title will be of the form "ModelComparisons.ModelFits.ACD.mmm.ext",
#' where "ext" is the appropriate file extension and "mmm" is a dash-separated string of model names.
#'
#' @export
#'
modelComparisons.ModelFits.ACD<-function(models,
                                            plot1stObs=TRUE,
  ...
  output_file<-paste0("ModelComparisons.ModelFits.ACD.",mmm,".",ext);
  title<-paste0("Model Model Comparisons: Fits to Aggregated Catch Data -- ",mmv);
